...
import type { FormNode, LayoutNode, PageSpec, ProjectProfile, Recipe, TableNode } from '../model/types';
import { compileRecipe } from '../recipes/recipe.apply';
  ...
  switch (node.type) {
    case 'PageRoot':
      const rootChildren = (node.children || [])
        .map((c: LayoutNode) => generateDSL(c, indent + 1))
        .join('\n');
    ...
    case 'Grid': {
      ...
      const children = (node.children || [])
        .map((c: LayoutNode, i: number) => `${pad}  C${i + 1}=${generateDSL(c, 0).trim()}`)
        .join('\n');
    ...
    case 'GridCell': {
      ...
      const children = (node.children || [])
        .map((c: LayoutNode) => `${pad}  ${generateDSL(c, 0).trim()}`)
        .join('\n');
      return `${pad}${header}${children ? `\n${children}` : ''}`;
    }

    case 'Table': {
      const cols = (node.columns || []).map((c: any) => c.key).join(',');
      const filters = node.filters?.map((f: any) => f.key).join(',') || '';
    ...
    case 'Form': {
      const fields = (node.fields || []).map((f: any) => f.key).join(',');
    ...
    case 'Dialog': {
      const dialogContent = (node.children || [])
        .map((c: LayoutNode) => generateDSL(c, 0).trim())
        .join(', ');
